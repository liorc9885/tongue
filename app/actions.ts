...
export async function getLeaderboardByFastest(): Promise<ScoreEntry[]> {
  const supabaseUrl = process.env.SUPABASE_URL
  const supabaseKey = process.env.SUPABASE_KEY
  ...
  const res = await fetch(
    `${supabaseUrl}/rest/v1/scores?time_spent=gt.0&order=score.desc&limit=100&select=player_name,score,max_level,time_spent,created_at`,
    {
      headers: {
        apikey: supabaseKey,
        Authorization: `Bearer ${supabaseKey}`,
